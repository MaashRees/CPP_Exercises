...
1. Pourquoi n'y a-t-il pas de relation entre `last_wheel` et `wheels[3]` contrairement à `first_wheel` et `wheels[0]` ?
   ```
   Car last_wheel n'est pas une réference mais juste une variable qui cntient une copie de wheels[3]
   ```
...
1. Dans le graphe d'ownership, comment distingue-t-on un pointeur d'une référence ?
   ```
   Un pointeur est une bloc de données avec une flèche en pointillé pour faire référence à une donnée (*)
   Une référence est un bloc de donnée en pointillé(car il existe pas vraiment,mais est plutôt un alias) avec une flèche en pointillé vers la donnée dont il fait référence (&)
   ```
...
3. En termes de code, quelles sont les deux différences principales entre un pointeur-observant et une référence ?
   ```
   - Réassignable : on peut réassigner un pointeur a une autre valeur, mais pas une réference
   - Nullité : pointeur peut être null mais pas une réference
   ```
...
2. Quel problème relève-t-on dans le graphe ?
   ```
   first_product pointe toujours vers l'ancienne zone mémoirede products[0] alors qu'il a été déplacé et on peut y avoir n'importe quoi```
...
4. Redessinez le graphe d'ownership de la question 1, mais en prenant en compte vos changements dans le code.
     ![](images/C_4.png)
6. Avez-vous toujours le problème relevé à la question 2 ? Non
...
// car on a pas besoin de modifier (const) et pas besoin de copie donc (&)
