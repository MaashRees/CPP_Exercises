...
int main()
{
    ...
    tiger.move();                  // I1 TD :  TS :  Fonction appelée : move de tiger
    tiger_as_animal.move();        // I2 move de animal car virtual

    bird.move();                   // I3 move de animal car virtual
    bird_as_animal.move();         // I4 move de animal

    bird.can_eat_plant();          // I5
    bird_as_herb.can_eat_plant();  // I6
    bird_as_carn.can_eat_plant();  // I7 carnivore car pas virtual

    tiger.can_eat_plant();         // I8
    tiger.can_eat_meat();          // I9 celui de tiger car il cache celui de carnivore
    tiger_as_carn.can_eat_meat();  // I10

    bird.type();                   // I11 bird
    bird_as_animal.type();         // I12 animal car no virtual
    tiger_as_animal.type();        // I13 animal car 
...
- Dans [Dungeon.cpp](dungeon/Dungeon.cpp), quel est le rôle de la fonction `display` ? elle affiche la map du dongon avec les entitées
- Dans [Dungeon.cpp](dungeon/Dungeon.cpp), quel est le rôle de la fonction `update` ? elle met à jour les entitées de la map (déplacements, mort, ..)et la map elle même 
- Quelle variable du `main` porte l'ownership des entitées ? all_entities
...
```
mettre get_representation dans Entity à virtual et redefinir dans Character avec override
```
...
```
mettre update en virtual et le redefinir pour trap sans rien faire
```
